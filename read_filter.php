<?php 
    ...
    WHERE puli_rest_time.Day_ID = '".$today_date."' ";
    ...
    if ($max_price != NULL && $min_price != NULL) {
        $f_sql = $f_sql . "AND restaurant_price >= '" . $min_price . "' 
        AND restaurant_price <= '" . $max_price . "'";
    }
    ...
    if ($result -> num_rows > 0) {
        ...
        if ($dist != NULL && count($arr_fl_data) != NULL){
            for ($i = 0; $i < count($arr_fl_data); $i++){
                //PHP代碼以檢索JSON數據 
                @$distance_data = file_get_contents('https://maps.googleapis.com/maps/api/distancematrix/json?&origins='.urlencode($user_location).'&destinations='.urlencode($arr_fl_data[$i]['Restaurant_address']).'&key=AIzaSyAE86ozbw5PYKeYzhTaZ71buMjLMozzc_U');
